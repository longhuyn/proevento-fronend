...
export default class Feed extends Component {
    ...
    loadEvents() {
        axios.get("http://proevento.tk:3000/event/public").then((res) => {
            if (res.status === 200) {
                this.setState({ eventList: res["data"] });
                let newList = [];
                let curUser = localStorage.getItem("user");
                for (var i=0; i< this.state.eventList.length; i++) {
                    if (this.state.eventList[i].favorites.includes(curUser)) {
                        newList.push(this.state.eventList[i]);
                    }
                }
    ...
    render() {
        return (
            <div>
                <h3 className="text-center">Favorite Feed</h3>
